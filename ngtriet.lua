...
-- PHẦN CHAT COMMANDS PHÍA DƯỚI GIỮ NGUYÊN KHÔNG THAY ĐỔI
...
local function Find(n)
	...
	for _,p in ipairs(P:GetPlayers()) do
		if p~=L and (p.Name:lower():find(n) or p.DisplayName:lower():find(n)) then return p end
	end
end
...
T.MessageReceived:Connect(function(m) local s=m.TextSource if s and P:GetPlayerByUserId(s.UserId)==L then Cmd(m.Text) end end)
...
R.Stepped:Connect(function() if not E then return end local C=L.Character if not C then return end for _,v in ipairs(C:GetDescendants()) do if v:IsA("BasePart") then v.CanCollide=false end end end)
...
task.spawn(function()
	while true do
		...
		if E and TG then
			...
			if R1 and R2 and (R1.Position-R2.Position).Magnitude<25 then Tap(825,315) task.wait(.02) Tap(895,315) end
		end
	end
end)
...
R.Heartbeat:Connect(function()
	...
	if E and TG and BV then
		...
		Dir=Dir.Unit
		if Dist>TPD then BV.Velocity=Dir*((Dist<40 and NF) or HF)
		else
