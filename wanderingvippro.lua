...
local bHB = Btn(Frames[2], "HITBOX VÀ LOẠI TRỪ", 0, 230, Color3.fromRGB(100, 100, 255)); local hl, hd, hu = Adj(Frames[2], 35); local Scroll = Instance.new("ScrollingFrame", Frames[2]); Scroll.Size = UDim2.new(1, 0, 1, -80); Scroll.Position = UDim2.new(0, 0, 0, 70); Scroll.BackgroundColor3 = Color3.fromRGB(25, 25, 30); Scroll.ScrollBarThickness = 4; local Layout = Instance.new("UIListLayout", Scroll)
local function UpdateList() for _, v in pairs(Scroll:GetChildren()) do if v:IsA("TextButton") then v:Destroy() end end; for _, p in pairs(game.Players:GetPlayers()) do if p ~= Player then local b = Instance.new("TextButton", Scroll); b.Size = UDim2.new(1, -10, 0, 28); b.Text = p.DisplayName; b.BackgroundColor3 = IgnoreList[p.Name] and Color3.fromRGB(150, 50, 50) or Color3.fromRGB(40, 40, 45); b.TextColor3 = Color3.new(1, 1, 1); Instance.new("UICorner", b); b.MouseButton1Click:Connect(function() IgnoreList[p.Name] = not IgnoreList[p.Name]; UpdateList() end) end end; Scroll.CanvasSize = UDim2.new(0, 0, 0, Layout.AbsoluteContentSize.Y) end
...
local function UI() 
    ...
    bTele.BackgroundColor3 = IsTelePro and Color3.new(0, 0.8, 0) or Color3.fromRGB(100, 0, 0); bAtk.BackgroundColor3 = IsAutoClick and Color3.fromRGB(173, 216, 230) or Color3.fromRGB(100, 60, 0); bAnti.BackgroundColor3 = IsAntiStun and Color3.fromRGB(144, 238, 144) or Color3.fromRGB(0, 80, 0); bAnti.Text = IsAntiStun and "WANDER VB VIPPRO: BẬT" or "WANDER VB VIPPRO: TẮT"; bWS.Text = "TỐC ĐỘ " .. (IsSpeedHack and "BẬT" or "TẮT"); bWS.BackgroundColor3 = IsSpeedHack and Color3.new(1, 0, 1) or Color3.fromRGB(150, 0, 150); bNc.BackgroundColor3 = IsNoclip and Color3.new(0.6, 0.6, 0.6) or Color3.fromRGB(45, 45, 45); bIf.BackgroundColor3 = IsInfJump and Color3.new(0, 0.8, 0) or Color3.fromRGB(0, 100, 0); bFarm.BackgroundColor3 = AutoFarmActive and Color3.fromRGB(0, 150, 0) or Color3.fromRGB(200, 0, 0); bE.BackgroundColor3 = SpamEActive and Color3.fromRGB(0, 150, 0) or Color3.fromRGB(200, 0, 0); wl.Text = "T/ĐỘ " .. CurrentWS; xl.Text = "X " .. OX; yl.Text = "Y " .. OY; zl.Text = "Z " .. OZ; hl.Text = "HB " .. HBS 
end

Toggle.MouseButton1Click:Connect(function() Main.Visible = not Main.Visible end); bAnti.MouseButton1Click:Connect(function() IsAntiStun = not IsAntiStun; UI() end); for i, b in pairs(Tabs) do b.MouseButton1Click:Connect(function() for j, f in pairs(Frames) do f.Visible = (j == i) end end) end; bTele.MouseButton1Click:Connect(function() IsTelePro = not IsTelePro; UI() end); bMode.MouseButton1Click:Connect(function() BMode = (BMode % 3) + 1; UI() end); bAtk.MouseButton1Click:Connect(function() IsAutoClick = not IsAutoClick; UI(); if not IsAutoClick and CombatRemote then pcall(function() CombatRemote:FireServer("Block", false); ActionRemote:FireServer("Block", false) end) end end); bWS.MouseButton1Click:Connect(function() IsSpeedHack = not IsSpeedHack; UI() end); bNc.MouseButton1Click:Connect(function() IsNoclip = not IsNoclip; UI() end); bIf.MouseButton1Click:Connect(function() IsInfJump = not IsInfJump; UI() end); bHB.MouseButton1Click:Connect(function() IsHitboxAll = not IsHitboxAll end); bFarm.MouseButton1Click:Connect(function() AutoFarmActive = not AutoFarmActive; UI() end); bE.MouseButton1Click:Connect(function() SpamEActive = not SpamEActive; UI() end); wu.MouseButton1Click:Connect(function() CurrentWS += 10; UI() end); wd.MouseButton1Click:Connect(function() CurrentWS = math.max(16, CurrentWS - 10); UI() end); xu.MouseButton1Click:Connect(function() OX += 2; UI() end); xd.MouseButton1Click:Connect(function() OX -= 2; UI() end); yu.MouseButton1Click:Connect(function() OY += 2; UI() end); yd.MouseButton1Click:Connect(function() OY -= 2; UI() end); zu.MouseButton1Click:Connect(function() OZ += 2; UI() end); zd.MouseButton1Click:Connect(function() OZ -= 2; UI() end)
...
Player.CharacterAdded:Connect(function() IsForceRespawn = false end); UI(); UpdateList(); game.Players.PlayerAdded:Connect(function() task.wait(1); UpdateList() end); game.Players.PlayerRemoving:Connect(function(player) IgnoreList[player.Name] = nil; UpdateList() end)
