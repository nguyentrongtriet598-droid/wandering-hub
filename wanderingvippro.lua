...
RunService.Stepped:Connect(function() 
    ...
    if IsTelePro and t and t.Character and t.Character:FindFirstChild("HumanoidRootPart") and not (isStunned and not recentlySpawned) then 
        ...
        if direction.Magnitude > 2 then hrp.AssemblyLinearVelocity = direction.Unit * 260 -- Tốc độ đã tăng lên 260
...
Player.CharacterAdded:Connect(function() SpawnTime = tick(); IsForceRespawn = false; UI() end); UI(); UpdateList(); game.Players.PlayerAdded:Connect(function() task.wait(1); UpdateList() end); game.Players.PlayerRemoving:Connect(function(player) IgnoreList[player.Name] = nil; UpdateList() end)
