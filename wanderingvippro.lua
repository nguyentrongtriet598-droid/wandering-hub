...
-- Phần Tab HITBOX có nút điều chỉnh kích thước riêng biệt
local bHB = Btn(Frames[2], "HITBOX VÀ LOẠI TRỪ", 0, 230, Color3.fromRGB(100, 100, 255))
local hl, hd, hu = Adj(Frames[2], 35) -- Khởi tạo nút hiển thị và chỉnh kích cỡ hitbox ở Tab 2
...
local function UpdateList() for _, v in pairs(Scroll:GetChildren()) do if v:IsA("TextButton") then v:Destroy() end end; for _, p in pairs(game.Players:GetPlayers()) do if p ~= Player then local b = Instance.new("TextButton", Scroll); b.Size = UDim2.new(1, -10, 0, 28); b.Text = p.DisplayName; b.BackgroundColor3 = IgnoreList[p.Name] and Color3.fromRGB(15, 50, 50) or Color3.fromRGB(40, 40, 45); b.TextColor3 = Color3.new(1, 1, 1); Instance.new("UICorner", b); b.MouseButton1Click:Connect(function() IgnoreList[p.Name] = not IgnoreList[p.Name]; UpdateList() end) end end; Scroll.CanvasSize = UDim2.new(0, 0, 0, Layout.AbsoluteContentSize.Y) end
...
-- Sự kiện nhấn nút tăng giảm Hitbox (+) và (-) ở Tab HITBOX
